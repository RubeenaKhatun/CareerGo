            ...
            <section key={tier.name}>
              <div className='px-4 mb-8'>
                ...
                <p>
                  <span className='text-4xl font-extrabold text-gray-900'>
                    ...
                    {tier.price > withoutGST(payment?.planPrice, payment?.plan)
                      ? Math.max(
                          0,
                          tier.price - (payment ? withoutGST(payment?.planPrice, payment?.plan) : 0)
                        )
                      : tier.price}
                  </span>
                ...
                <Link
                  href={
                    payment
                      ? withoutGST(payment?.planPrice, payment?.plan) < tier.price
                  ...
                  >
                    {payment
                      ? payment.plan === tier.name
                        ? "Purchased"
                        : (tier.price > payment.planPrice ? "Upgrade to " : "Buy ") + tier.name
                      : "Buy " + tier.name}
                  </button>
                </Link>
              </div>
                  ...
                  <td key={tier.name} className='h-full py-2 px-6 align-top'>
                    <div className='h-full flex flex-col justify-between'>
                          ...
                          <span className='text-3xl font-extrabold text-gray-900'>
                            ...
                            {tier.price > withoutGST(payment?.planPrice, payment?.plan)
                              ? Math.max(
                                  0,
                                  tier.price -
                                    (payment ? withoutGST(payment?.planPrice, payment?.plan) : 0)
                                )
                              : tier.price}
                          </span>
                          <span className='text-base font-medium text-gray-500'>/year</span>
                      ...
                      >
                        ...
                        >
                          {console.log("tier ",tier.price, withoutGST(payment?.planPrice, payment?.plan))}
                          {payment
                            ? payment.plan === tier.name
                              ? "Purchased"
                              : (tier.price > withoutGST(payment?.planPrice, payment?.plan)
